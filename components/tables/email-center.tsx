...
export function EmailCenter({
  ...
  const [selectedIds, setSelectedIds] = useState<string[]>(initialStudentId ? [initialStudentId] : []);
  ...
  async function sendEmail() {
    ...
    if (!subject.trim() || !body.trim()) {
      setStatus("Add both a subject and email body.");
      return;
    }
    ...
    startTransition(async () => {
      ...
      const result = (await response.json().catch(() => null)) as
        | { error?: string; deliveryMode?: "logged" | "resend"; sentCount?: number; recipients?: Array<{ id: string }> }
        | null;

      if (!response.ok) {
        setStatus(result?.error ?? "Could not process email campaign.");
        return;
      }

      if (result?.deliveryMode === "resend") {
        setStatus(`Email campaign sent successfully to ${result.sentCount ?? 0} recipient(s).`);
      } else {
        setStatus("Emails were logged in the CRM. Add RESEND_API_KEY and EMAIL_FROM to enable direct delivery.");
      }
  ...
  return (
    <div className="space-y-6">
      <Card className="border-[#e8d7c6] bg-[linear-gradient(135deg,#fffaf4_0%,#fff2e6_100%)] dark:border-white/10 dark:bg-[linear-gradient(160deg,rgba(15,23,42,0.96),rgba(30,41,59,0.92))]">
        <CardHeader
          title="Email Center"
          description="Compose emails, reuse templates, and keep a complete outbound email log per student."
        />
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-4">
            ...
            <label className="block text-sm text-slate-600 dark:text-slate-300">
              <span className="mb-2 block font-medium text-ink dark:text-white">Email Body</span>
              <textarea
                value={body}
            ...
            <div className="flex flex-wrap gap-3">
              <Button type="button" onClick={sendEmail} disabled={isPending}>
                <Send className="mr-2 h-4 w-4" />
                {isPending ? "Processing..." : "Send / Log Email"}
              </Button>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {selectedStudents.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 px-4 py-8 text-sm text-slate-500 dark:border-white/10 dark:text-slate-400">
                  Select students to send or log an email campaign.
                </div>
              ) : (
      ...
      <Card className="dark:border-white/10 dark:bg-[#0d1729]">
        <CardHeader
          title="Recipients"
          description={`${students.length} students with email addresses are available for email outreach.`}
          action={
      ...
      <Card className="dark:border-white/10 dark:bg-[#0d1729]">
        <CardHeader
          ...
          description="Recent outbound emails recorded in the CRM."
          action={
              ...
              <div key={log.id} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.04]">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{log.student?.email ?? "No email"}</p>
                  </div>
