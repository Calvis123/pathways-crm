...
export function EmailCenter({
  ...
  const recipientsPageCount = Math.max(1, Math.ceil(filteredStudents.length / ITEMS_PER_PAGE));
  const safeRecipientsPage = Math.min(recipientsPage, recipientsPageCount - 1);
  const paginatedStudents = filteredStudents.slice(
    safeRecipientsPage * ITEMS_PER_PAGE,
    safeRecipientsPage * ITEMS_PER_PAGE + ITEMS_PER_PAGE
  );
  const logsPageCount = Math.max(1, Math.ceil(visibleLogs.length / ITEMS_PER_PAGE));
  const safeLogsPage = Math.min(logsPage, logsPageCount - 1);
  const paginatedLogs = visibleLogs.slice(safeLogsPage * ITEMS_PER_PAGE, safeLogsPage * ITEMS_PER_PAGE + ITEMS_PER_PAGE);
    ...
    <div className="space-y-6">
      <Card className="border-[#e8d7c6] bg-[linear-gradient(135deg,#fffaf4_0%,#fff2e6_100%)] dark:border-white/10 dark:bg-[linear-gradient(160deg,rgba(15,23,42,0.96),rgba(30,41,59,0.92))]">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
              ...
              <>
                ...
                <label className="block text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <textarea
                    ...
                    className="min-h-44 w-full rounded-lg border border-[#eadacc] bg-white px-4 py-3 dark:border-white/10 dark:bg-white/[0.06] dark:text-white dark:placeholder:text-slate-400"
          ...
          <div className="rounded-xl border border-[#eadacc] bg-white/80 p-5 dark:border-white/10 dark:bg-white/[0.05]">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-[linear-gradient(135deg,#ff7a59,#e09a54)] text-white">
                {channel === "email" ? (
            ...
            <div className="mt-5 space-y-3">
              {selectedStudents.length === 0 ? (
                ...
                selectedStudents.slice(0, 6).map((student) => (
                  <div key={student.id} className="rounded-lg border border-[#eadacc] bg-[#fff6ef] px-4 py-3 dark:border-white/10 dark:bg-white/[0.04]">
        ...
        <Card className="dark:border-white/10 dark:bg-[#182638]">
          ...
          <div className="grid gap-3 xl:grid-cols-2">
            {recipientStatuses.map((item) => (
              <div key={item.id} className="rounded-lg border border-[#eadacc] bg-[#fff6ef] p-4 dark:border-white/10 dark:bg-white/[0.04]">
      ...
      <Card className="dark:border-white/10 dark:bg-[#182638]">
        ...
        {filteredStudents.length > ITEMS_PER_PAGE ? (
          <div className="mb-4">
            <PaginationControls
              page={safeRecipientsPage}
              pageCount={recipientsPageCount}
          ...
          {visibleLogs.length === 0 ? (
            ...
            <>
              ...
              {paginatedLogs.map((log) => (
                <div key={log.id} className="rounded-lg border border-[#eadacc] bg-[#fffaf5] p-4 dark:border-white/10 dark:bg-white/[0.04]">
