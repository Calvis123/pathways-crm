...
export function DocumentsManager({
  ...
  const pageCount = Math.max(1, Math.ceil(groupedDocuments.length / ITEMS_PER_PAGE));
  const safePage = Math.min(page, pageCount - 1);
  const paginatedGroups = groupedDocuments.slice(safePage * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE + ITEMS_PER_PAGE);
  ...
  async function submitReview(formData: FormData) {
    ...
    const response = await fetch(`/api/documents/${reviewTarget.id}`, {
      method: "PATCH",
    ...
    <div className="space-y-6">
      <Card className="dark:border-white/10 dark:bg-[#182638]">
        ...
        <form action={handleUpload} className="grid gap-4 md:grid-cols-2">
          <select name="student_id" required className="rounded-lg border border-[#eadacc] px-4 py-3 dark:border-white/10 dark:bg-white/[0.06] dark:text-white">
          ...
          <select name="document_type" required className="rounded-lg border border-[#eadacc] px-4 py-3 dark:border-white/10 dark:bg-white/[0.06] dark:text-white">
          ...
          <label className="md:col-span-2 block rounded-xl border-2 border-dashed border-gold/30 bg-[#fff6ef] px-6 py-8 text-center transition hover:border-gold hover:bg-gold/5 dark:bg-white/[0.04] dark:hover:bg-white/[0.06]">
      ...
      <Card className="dark:border-white/10 dark:bg-[#182638]">
        ...
        {groupedDocuments.length === 0 ? (
          <div className="rounded-xl border border-dashed border-[#d9c1ad] px-6 py-16 text-center text-sm text-slate-500 dark:border-white/10 dark:text-slate-400">
            No documents uploaded yet.
          </div>
        ) : (
          <div className="space-y-5">
            {groupedDocuments.length > ITEMS_PER_PAGE ? (
              <PaginationControls
                page={safePage}
                pageCount={pageCount}
            ...
            {paginatedGroups.map((group) => {
              ...
              return (
                <section key={group.studentId} className="rounded-xl border border-gold/20 bg-[#fffaf5] p-5 dark:border-white/10 dark:bg-white/[0.04]">
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    {group.docs.map((document) => (
                      <div key={document.id} className="rounded-xl border border-[#f0dfd0] bg-white p-5 dark:border-white/10 dark:bg-white/[0.05]">
                        ...
                        {document.review_notes ? (
                          <div className="mt-4 rounded-lg bg-[#fff6ef] p-3 text-sm italic text-slate-600 dark:bg-white/[0.04] dark:text-slate-300">
                            {document.review_notes}
                          </div>
                        ) : null}
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {document.file_url ? (
                            <>
                              <a href={document.file_url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-ink ring-1 ring-slate-200 dark:bg-white/[0.06] dark:text-white dark:ring-white/10">
                                View
                              </a>
                              <a href={document.file_url} download className="inline-flex items-center justify-center rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-ink ring-1 ring-slate-200 dark:bg-white/[0.06] dark:text-white dark:ring-white/10">
                                Download
                              </a>
                            </>
                          ) : null}
      ...
      {reviewTarget ? (
        ...
        >
          <div className="w-full max-w-lg rounded-xl border border-[#eadacc] bg-white p-6 shadow-panel dark:border-white/10 dark:bg-[#0f1b31]">
            ...
            <form action={submitReview} className="space-y-4">
              <textarea
                ...
                className="w-full rounded-lg border border-[#eadacc] px-4 py-3 dark:border-white/10 dark:bg-white/[0.06] dark:text-white dark:placeholder:text-slate-400"
              />
